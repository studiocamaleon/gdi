...
export type TapBounds = { minX: number; minY: number; maxX: number; maxY: number };
...
function pointEquals(a: TapPoint | null, b: TapPoint | null, tolerance = 1e-6): boolean {
  return Boolean(a && b && distance(a, b) <= tolerance);
}

export function analyzeTap(tap: string): TapAnalysis {
  ...
  let feedRateMmPerMin: number | null = null;
  ...
  for (const rawLine of lines) {
    ...
    if (!startedCoordinates) headerLines.push(rawLine.replace(/\r$/, ""));
    const feed = line.match(/(?:^|\s)F([-+]?\d*\.?\d+)/i);
    if (feed) feedRateMmPerMin = Number(feed[1]);
  }
  ...
  for (let i = 1; i < points.length; i += 1) {
    ...
    if (segment <= 1e-9) zeroLengthMoves += 1;
  }

  let bounds: TapBounds | null = null;
  if (points.length > 0) {
    bounds = points.reduce<TapBounds>(
      (acc, point) => ({
...
function main(): void {
  ...
  if (!input) throw new Error("Uso: node dist/tap-analyzer.js <archivo.tap> [salida.json]");
