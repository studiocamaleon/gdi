...
- **DNS/HTTPS:** Donweb tiene A/AAAA directos para `staging`, CNAME para `api-staging`, TXT de propiedad y CNAME para los desafíos ACME. Los registros responden correctamente en los DNS autoritativos y en las comprobaciones desde Fly. El emisor alterna entre `Awaiting configuration` y `Awaiting certificates`; todavía no emitió los certificados. **El acceso por los dominios propios no está habilitado hasta que HTTPS pase la verificación.**
