...
import { getDuracionesFamilias, getEstaciones, type DuracionFamilia } from "@/lib/estaciones-api";
 ...
 * vista Por estación (familia → estación) y las medianas históricas por
 * familia (fallback de la cola en horas). Si la API no responde, la vista
...
export default async function TableroProduccionPage() {
  ...
  try {
    [{ items }, estaciones, duraciones] = await Promise.all([
  ...
  return <TableroProduccion initialItems={items} estaciones={estaciones} duracionesFamilias={duraciones} />;
}
