import type { SubfamiliaMateriaPrima, UnidadMateriaPrima } from "@/lib/materias-primas";

const FLEXIBLE_ROLL_SUBFAMILY = "sustrato_rollo_flexible";
const FLEXIBLE_ROLL_SUPPORTED_UNITS = new Set<UnidadMateriaPrima>(["rollo", "m2", "metro_lineal"]);
...
export function canUseFlexibleRollDerivedUnits(input: {
  ...
  from?: UnidadMateriaPrima | null;
  to?: UnidadMateriaPrima | null;
  attributes?: unknown;
}) {
...
export function convertFlexibleRollUnitPrice(input: {
  ...
  from: UnidadMateriaPrima;
  to: UnidadMateriaPrima;
  attributes?: unknown;
  subfamilia?: SubfamiliaMateriaPrima | string | null;
}) {
