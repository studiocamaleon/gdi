...
export async function getEmpleados() {
  return request<EmpleadoDetalle[]>("/empleados");
}

export async function getEmpleadoById(id: string) {
  try {
    return await request<EmpleadoDetalle>(`/empleados/${id}`);
  } catch (error) {
...
export async function updateEmpleado(id: string, payload: EmpleadoPayload) {
  return request<EmpleadoDetalle>(`/empleados/${id}`, {
    method: "PUT",
    body: JSON.stringify(payload),
  });
}
...
export async function deleteEmpleado(id: string) {
  return request<void>(`/empleados/${id}`, {
    method: "DELETE",
  });
}
