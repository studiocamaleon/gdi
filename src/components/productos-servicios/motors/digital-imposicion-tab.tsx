...
function getGuillotinaCutsFromImposicion(cols: number, rows: number, tipoCorte: "sin_demasia" | "con_demasia") {
  ...
  const baseCuts = Math.max(0, cols - 1) + Math.max(0, rows - 1);
  return tipoCorte === "con_demasia" ? baseCuts + 2 : baseCuts;
}
...
export function DigitalImposicionTab(props: ProductTabProps) {
  ...
  React.useEffect(() => {
    if (!props.selectedVariant) {
      setConfig(mergeImposicionConfig(props.motorConfig?.parametros, null));
    ...
    getVarianteMotorOverride(props.selectedVariant.id)
      .then((overrideConfig) => {
        if (cancelled) return;
        setConfig(mergeImposicionConfig(props.motorConfig?.parametros, overrideConfig.parametros ?? null));
      })
      .catch(() => {
        if (cancelled) return;
        setConfig(mergeImposicionConfig(props.motorConfig?.parametros, null));
  ...
  const handleSaveConfig = () => {
    ...
    startSavingConfig(async () => {
      try {
        ...
        setConfig(mergeImposicionConfig(props.motorConfig?.parametros, updated.parametros ?? null));
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        ...
        <Button type="button" onClick={handleSaveConfig} disabled={isSavingConfig || !props.selectedVariant}>
