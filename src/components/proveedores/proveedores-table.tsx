...
import { createProveedor, deleteProveedor } from "@/lib/proveedores-api";
  ...
  const handleImportFile = (file: File | undefined) => {
    ...
    startImportTransition(async () => {
      ...
      for (let idx = 0; idx < payloads.length; idx += 1) {
        try {
          created.push(await createProveedor(payloads[idx]));
        } catch (error) {
          failed.push(
      ...
      if (failed.length > 0) {
        toast.error(
          `Se importaron ${created.length} proveedor(es), con ${failed.length} error(es). ${failed[0]}`,
        );
      } else {
        toast.success(`Se importaron ${created.length} proveedor(es).`);
      }
    });
  };
