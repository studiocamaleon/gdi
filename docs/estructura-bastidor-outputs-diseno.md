...
| 2026-08-13 | **Ola #2 — 1ª rebanada (mueve precios):** mecanismo `publicaCanon` (el derivador declara qué geometría de su traza escribe al JobContext; el motor la promueve tras correr el derivador, ambas ramas) + JobContext gana `interiorMm`/`lonaBrutaMm`. El LED lee `interiorMm` con prioridad → siembra sobre el interior (96, no 100). Golden master **re-baselineado**: sólo se movió el LED (−$125,44 constante = cable sobre el perímetro interior; el conteo de módulos quedó igual porque el `ceil` de la grilla absorbió el interior en 2,4×1,2; el spec 2510→2490 prueba que sí baja cuando cruza un escalón). Estructura idéntica. 2 specs LED nuevos; 12 fallos de motor.spec siguen siendo los pre-existentes. Falta conectar lona (lee `lonaBrutaMm`) y chapa/cenefa (panelización). |
