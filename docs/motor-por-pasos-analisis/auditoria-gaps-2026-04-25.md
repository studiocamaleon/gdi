...
- **2026-04-26 (v3.0)**: ✅ **Cleanup completo de plantillas + alineación a doc §5–§13**.
  - **Schema**: enum `PlantillaMaquinaria` reducido de 24 a 12 plantillas finales (eliminadas 8 viejas de impresoras unificadas en `IMPRESORA_GRAN_FORMATO_POR_AREA` con discriminantes tecnologia + geometria; eliminadas INYECCION_TINTA, 3D, UV_CILINDRICA, PERFORADORA, REDONDEADORA_PUNTAS por no aplicar al rubro o ser familias). Eliminados 2 enums legacy (`TipoImpresionProductoVariante`, `CarasProductoVariante`). Eliminadas **10 columnas legacy** de `MaquinaPerfilOperativo` (`printMode`, `printSides`, `cantidadPasadas`, `dobleFaz`, `materialPreset`, `sheetThicknessMm`, `maxBatchHeightMm`, `operationMode`, `anchoAplicable`, `altoAplicable`) — todas migradas a `detalleJson`. Agregada columna universal `gramajeMaxGr` y `reglaSeleccionJson`.
  - **Backend**: `maquinaria.service.ts` + DTO + `template-machine-rules` + `template-profile-rules` reescritos para v3.0. Cada plantilla declara qué campos son obligatorios y qué claves de detalle permite (validación por schema).
  - **Motor**: `nesting-dispatcher.ts` ahora lee `paramsTecnicos.anchoMaxRolloMm` (modelo nuevo) con fallback a `anchoMaxMm`, y aplica `margenesNoImprimiblesMm.{izq,der}` para reducir el ancho útil del rollo + `.sup`/`.inf` para márgenes vertical (cierre del bug original que motivó esta sesión).
  - **Seed v3.0**: Roland VG3-540 ahora es `IMPRESORA_GRAN_FORMATO_POR_AREA` con `tecnologia=LATEX + geometria=ROLLO + margenesNoImprimiblesMm`. Mimaki UJF-7151 idem con `tecnologia=UV + geometria=MESA_EXTENSORA`. Polar 92 con 3 perfiles por rango de gramaje + `pliegosMaxPorTanda`. Skycut con perfiles por `tipoCorte + factorComplejidad`. Discriminantes en `detalleJson` según doc §5–§13.
  - **Frontend**: `lib/maquinaria.ts` limpio (12 plantillas, 4 familias, sin tipos legacy). `lib/maquinaria-templates.ts` reescrito desde cero (12 templates con campos del doc, sin inventar). `lib/materia-prima-templates.ts` con 12 keys. `maquinaria-panel.tsx` reemplazado por componente nuevo template-driven (renderiza dinámicamente las secciones del template, sin hardcodear plantillas). De 6266 LOC a ~600 LOC.
  - **Validación end-to-end**: vinilo 3 paños 2x1m → ahora con ancho útil 1360mm (1370 rollo - 10 márgenes), aprovechamiento 73.2%, 6.03 m lineales consumidos. **El bug de márgenes del nesting cerrado.**
  - **DB reseteada via `prisma db push --force-reset`**: data demo regenerada con seed v3.0 limpio.
  - 120/120 tests verde + typecheck + build verde.
...
- **2026-04-25 (8)**: ✅ **G-M9 + G-M7 + G-S2 + G-M8 cerrados** (4 gaps backend en una pasada).
  ...
  - **G-S2** (plantillas pendientes): SOLDADORA, CABINA_PINTURA y ANILLADORA agregadas al enum `PlantillaMaquinaria` (migración Prisma `g_s2_plantillas_soldadora_cabina_anilladora`). Type frontend extendido + 3 templates mínimos en `maquinaria-templates.ts` reusando estructura de Guillotina.
  - **G-M8** (perfil con regla declarativa): `resolverPerfil` ahora soporta JsonLogic en `perfil.detalleJson.reglaSeleccion` (o `condicion`). Itera perfiles activos, evalúa la regla contra el JobContext y elige el primer match. Heurística legacy (impresion_por_hoja según caras) queda como fallback.
  - 120/120 tests verde (5 nuevos del sprint).
...
| ID | Gap | Donde se ve | Impacto |
|---|---|---|---|
| ~~**G-M7**~~ | ~~`MAYOR_APROVECHAMIENTO` solo ordena por anchoMm~~ | ✅ **CERRADO 2026-04-25** | Dispatcher corre con cada candidato y elige el de mayor aprovechamientoPct. |
