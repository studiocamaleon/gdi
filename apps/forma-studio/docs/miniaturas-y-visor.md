...
Las once tarjetas disponibles usan renders propios de los sólidos del motor. Un bloque de esquinas biseladas con un corte frontal permite ver paredes, apoyos y tapas; el naranja identifica la sección del cuerpo. Letra curva usa el texto «3D», Especiales muestra el perfil Acanalado y Frente calado muestra hexágonos con el difusor debajo. Son ejemplos de cada familia, no una previsualización de los parámetros del proyecto abierto.

Las imágenes están en `public/style-previews/`, a 512 × 384, con transparencia. Las tarjetas usan `<img>` y no crean visores WebGL adicionales. El bloque biselado reemplaza las anteriores vistas semicirculares.

`AVAILABLE_STYLES` define el catálogo visible y las miniaturas a regenerar. Doble iluminación y Canal neón permanecen en el registro interno para poder abrir proyectos anteriores, pero no se ofrecen en el selector.
