...
function sweepChangingTopology(
  ...
  for (let i = 1; i < samples.length; i++) {
    ...
    finishRun();
    // Un offset puede borrar la sección entera. CrossSection no admite [];
    // el puente conserva sólo el extremo que todavía contiene material.
    const bridge = keep(
      wasm.CrossSection.union(
        [a.polygons, b.polygons]
          .filter((polygons) => polygons.length > 0)
          .map((polygons) => keep(new wasm.CrossSection(polygons))),
      ),
    );
