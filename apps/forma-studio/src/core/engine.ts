...
export function buildModel(wasm: ManifoldToplevel, input: EngineInput): Model {
  ...
  function finish(
    ...
    solids.forEach((source, segment) => {
      ...
      function serialize(precision: number) {
        ...
        return { solid: m, mesh, collapsed: collapsed() };
      }
      let prepared = serialize(precision);
      if (prepared.collapsed) {
        // Algunas curvas conservan caras colineales al redondear la grilla
        // decimal a Float32. Reintentar desde el sólido original con una grilla
        // binaria exacta evita ese segundo redondeo, sin aumentar la tolerancia
        // ni acumular simplificaciones sobre la geometría del intento anterior.
        prepared = serialize(2 ** Math.floor(Math.log2(precision)));
      }
      if (prepared.collapsed)
        throw new Error(
          "La malla contiene caras colapsadas y no puede exportarse. Revisá las dimensiones del perfil.",
        );
      const { solid: m, mesh } = prepared;
