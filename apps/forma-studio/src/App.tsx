...
import { Grafoprint3dBrand } from "./components/Grafoprint3dBrand";
import {
  Box,
  Type,
  Upload,
...
import { newProject, chooseStyle, STYLES, AVAILABLE_STYLES } from "./core/project";
import {
  contoursFromSource,
  FONT_NAMES,
  physicalHeight,
...
import { editorDraft } from "./core/grafoprint-demo";
import {
  isFitStyle,
  componentLabel,
  type FitComponent,
...
function StylePreview({ id }: { id: string }) {
  return (
    <div className="style-preview" aria-hidden="true">
      <img
        src={publicAsset(`style-previews/${id}.png`)}
        alt=""
        width={512}
        height={384}
        decoding="async"
...
export default function App() {
  const [draft] = useState(() => editorDraft(window.location.search));
  const [project, setProject] = useState<Project>(() => {
    try {
      const p = localStorage.getItem(draft.key);
      return p ? parseProject(JSON.parse(p)) : draft.create();
    } catch {
      return draft.create();
    }
  });
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      try {
        localStorage.setItem(draft.key, JSON.stringify(project));
      } catch {
  ...
  return (
    <div className={cn("app-shell", dark && "dark")}>
      ...
      <div className="public-tool-bar">
        <span>Grafo3D gratis <span className="public-tool-detail">· Sin registro · Tus proyectos se guardan en este navegador</span></span>
        <a href="/" target="_blank" rel="noopener noreferrer">
          Conocé Grafoprint <ArrowRight aria-hidden="true" />
        </a>
      </div>
      <header className="app-header">
        <a
          className="wordmark"
          href="#"
          aria-label="Grafoprint 3D · Inicio"
          onClick={(e) => {
            e.preventDefault();
            setPage("design");
          }}
                ...
                {mode === "lightbox" ? <LightboxOverview project={project} model={busy?null:model} onChange={update}/> : mode === "joint" ? (
                  ...
                  <>
                    ...
                    <div className="style-heading">
                      <h3>Construcción</h3>
                      <span>{AVAILABLE_STYLES.length} estilos</span>
                    </div>
                    <div
            ...
            <aside className="inspector">
              ...
              <div className="inspector-footer">
                <Button
                  ...
                  aria-describedby="production-next-step"
                  onClick={() => setPage("production")}
                >
                ...
                <p id="production-next-step">
                  Organizá las piezas y descargá tus archivos.
                </p>
              </div>
            </aside>
          </main>
