...
import { createLetter } from "../src/core/letter-models";
...
it("exporta un perfil curvo de SVG sin colapsar caras al redondear a Float32", async () => {
  // Contorno anular aislado de un SVG: curvas muestreadas, pared estrecha y
  // coordenadas alejadas del origen. Con la grilla decimal quedaban tres
  // triángulos colineales incluso después de simplificar el sólido cerrado.
  const contours: Contours = JSON.parse(
    await readFile(
      new URL("./fixtures/annular-svg-contours.json", import.meta.url),
      "utf8",
    ),
  );
  const project = chooseStyle(newProject(), "organic");
  project.params.organicProfile = "bumper";
  ...
  try {
    const reference = createLetter(
      wasm,
      keep,
      keep(new wasm.CrossSection(contours)),
      project.style,
      project.params,
    );
    ...
    expect(model.parts.map((part) => part.layer)).toEqual(["body", "face"]);
    for (const part of model.parts) {
      const source = part.layer === "body" ? reference.body : reference.face!;
      const bounds = source.boundingBox();
      expect(
        Math.abs(part.volume - source.volume()) / source.volume(),
      ).toBeLessThan(0.0001);
      for (const side of ["min", "max"] as const)
        for (let axis = 0; axis < 3; axis++)
          expect(
            Math.abs(part.bounds[side][axis] - bounds[side][axis]),
          ).toBeLessThan(0.002);
      // El archivo debe seguir cerrado y sin triángulos de área cero también
      // después de orientarlo y ubicarlo sobre la mesa de impresión.
      for (const bytes of [
        stl([part], true),
        stl([
          placedPart({
        ...
        const reopened = keep(readStl(bytes));
        expect(reopened.status()).toBe("NoError");
        expect(
