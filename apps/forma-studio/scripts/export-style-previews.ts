...
import { AVAILABLE_STYLES, chooseStyle, newProject } from "../src/core/project";
...
wasm.setup();
// Módulo biselado propio, abierto por un corte frontal para ver su construcción.
const sample: Contours[] = [[[
  [-78, -60], [-58, -80], [58, -80], [78, -60],
  [78, 60], [58, 80], [-58, 80], [-78, 60],
]]];
const previews = [];
for (const style of AVAILABLE_STYLES) {
  ...
  // El relieve acanalado distingue Especiales incluso a tamaño de miniatura.
  if (style.id === "organic") {
