...
export function costingM2Exact<T = unknown>(input: CostingInput<T>): CostingResult {
  ...
  const pricePerM2Value = pricePerM2(input.unitPrice, substrate.widthMm, substrate.heightMm);
  const pieceWidthMm = input.pieceWidthMm ?? input.nesting.placements[0]?.widthMm ?? 0;
  const pieceHeightMm = input.pieceHeightMm ?? input.nesting.placements[0]?.heightMm ?? 0;
  const areaPiezasM2 = (pieceWidthMm * pieceHeightMm * input.totalPieces) / 1_000_000;
