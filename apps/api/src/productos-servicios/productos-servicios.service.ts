...
import * as RigidPrintedCalc from './motors/rigid-printed.calculations';
import { RigidPrintedMotorModule } from './motors/rigid-printed.motor';
...
import {
  getGranFormatoNullableNumber as getGranFormatoNullableNumberV2,
  buildGranFormatoPieceInstances as buildGranFormatoPieceInstancesV2,
  expandGranFormatoMeasuresToSinglePieces as expandGranFormatoMeasuresToSinglePiecesV2,
  buildGranFormatoPanelizedPieces as buildGranFormatoPanelizedPiecesV2,
  normalizeGranFormatoPanelManualLayout as normalizeGranFormatoPanelManualLayoutV2,
  buildGranFormatoManualPieces as buildGranFormatoManualPiecesV2,
} from './nesting/helpers/granformato-pieces';
import {
  evaluateGranFormatoMixedShelfLayout as evaluateGranFormatoMixedShelfLayoutV2,
...
export class ProductosServiciosService {
  ...
  async quoteRigidPrintedVariant(
    auth: CurrentAuth,
    varianteId: string,
    ...
    const { config, configVersionBase, configVersionOverride } = await this.getEffectiveMotorConfig(
      auth, variante.productoServicio.id, variante.id, motor,
    );
    const effectiveConfig = this.mergeMotorConfig(
      motor.code, config as Prisma.JsonValue, (payload.parametros ?? {}) as Record<string, unknown>,
    );

    const params = (payload.parametros ?? {}) as Record<string, unknown>;
    const rigidConfig = effectiveConfig as Record<string, unknown>;
    ...
    const variantesCompat = Array.isArray(rigidConfig.variantesCompatibles)
      ? (rigidConfig.variantesCompatibles as string[]) : [];
    const placaVarianteId = String(
      params.placaVarianteId ?? rigidConfig.placaVarianteIdDefault ?? variantesCompat[0] ?? '',
    );
    if (!placaVarianteId) {
      throw new BadRequestException('No hay placas configuradas. Asigná un material y variantes en el tab Tecnologías.');
    }
    ...
    const placaAttrs = (placaVariante.atributosVarianteJson ?? {}) as Record<string, unknown>;
    ...
    if (placaAnchoMm <= 0 || placaAltoMm <= 0) {
      throw new BadRequestException('La placa no tiene dimensiones válidas.');
    }
    ...
    const segmentos = Array.isArray(imposicionCfg.segmentosPlaca)
      ? (imposicionCfg.segmentosPlaca as number[])
      : [25, 50, 75, 100];
    ...
    let costoProcesos = 0;
    const bloquesProcesos: Array<{
      orden: number; codigo: string; nombre: string;
      centroCostoId: string | null; centroCostoNombre: string;
      setupMin: number; runMin: number; totalMin: number;
      tarifaHora: number; costo: number;
    }> = [];
    ...
    const snapshot = await this.prisma.cotizacionProductoSnapshot.create({
      data: {
        ...
        inputJson: ({ cantidad, periodo, anchoMm, altoMm, placaVarianteId, tipoImpresion, caras, config: effectiveConfig } as unknown) as Prisma.InputJsonValue,
   ...
   */
  async cotizarRigidPrintedByProducto(
    auth: CurrentAuth,
    productoId: string,
    payload: CotizarProductoVarianteDto,
  ) {
    ...
    // Buscar una variante existente o usar null
    ...
    const rigidConfig = this.mergeMotorConfig(
      motor.code, configRow?.parametrosJson, params,
    ...
    // Márgenes de máquina
    const mgConfig = (imposicionCfg.margenMaquina ?? {}) as Record<string, unknown>;
    ...
    // Nesting de placa rígida (siempre independiente)
  ...
  /** 2026-04-23 — Fase 1.5: delega a nesting/helpers/granformato-pieces.ts */
  private getGranFormatoNullableNumber(value: unknown) {
    return getGranFormatoNullableNumberV2(value);
  }
  ...
  private resolveDefaultMotorConfig(code: string): Record<string, unknown> {
    ...
    if (code === ProductosServiciosService.RIGID_PRINTED_MOTOR_DEFINITION.code) {
      return { ...DEFAULT_RIGID_PRINTED_CONFIG };
    }
    return {};
  }
  ...
  /** 2026-04-23 — Fase 1.5: delega a nesting/algorithms/shelf-rollo.ts */
  private buildGranFormatoNestingOrientacion(
    placements: Array<{ rotated: boolean }>,
  ): GranFormatoNestingOrientation {
    return buildGranFormatoNestingOrientacionV2(placements);
  }
  ...
  private countGranFormatoRowsAndPiecesPerRow(
    ...
    return countGranFormatoRowsAndPiecesPerRowV2(placements, toleranceMm);
  }
  ...
  private buildGranFormatoPieceInstances(
    ...
    return buildGranFormatoPieceInstancesV2(medidas);
  }
  ...
  private buildGranFormatoPanelizedPieces(input: {
    ...
    return buildGranFormatoPanelizedPiecesV2(input);
  }
  ...
  private normalizeGranFormatoPanelManualLayout(
    ...
    return normalizeGranFormatoPanelManualLayoutV2(value);
  }
  ...
  private buildGranFormatoManualPieces(input: {
    ...
    maxPanelWidthMm: number;
    widthInterpretation: 'total' | 'util';
    manualLayout: ReturnType<typeof normalizeGranFormatoPanelManualLayoutV2>;
  }) {
    if (!input.manualLayout) return null;
    return buildGranFormatoManualPiecesV2({
      medidas: input.medidas,
      printableWidthMm: input.printableWidthMm,
      maxPanelWidthMm: input.maxPanelWidthMm,
      widthInterpretation: input.widthInterpretation,
  ...
  /**
   * 2026-04-23 — Fase 1.5: delega a nesting/algorithms/shelf-rollo.ts.
   * El método se mantiene como wrapper privado para no romper los
   * call-sites internos (quoteVinylCutVariant, previewVinylCutVariant,
   * buildVinylCutSimulation, evaluateGranFormatoImposicionCandidates).
   */
  private evaluateGranFormatoMixedShelfLayout(input: {
    ...
    return evaluateGranFormatoMixedShelfLayoutV2(input);
  }
  ...
  private expandGranFormatoMeasuresToSinglePieces(
    ...
    return expandGranFormatoMeasuresToSinglePiecesV2(medidas);
  }
