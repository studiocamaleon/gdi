...
describe('Schema CRUD — Modelo Universal V2', () => {
  ...
  describe('Catálogo de máquinas', () => {
    it('hay 7 máquinas activas', async () => {
      if (!tenantId) return;
      const count = await prisma.maquina.count({ where: { tenantId, activo: true } });
      expect(count).toBe(7);
    });
    ...
    it('Ricoh PRO C5100 tiene 2 perfiles (simple/doble faz)', async () => {
      ...
      expect(nombres).toEqual(['Papel grueso doble faz', 'Papel grueso simple faz']);
    });
  });
  ...
  describe('Cargos directos catálogo (D.6)', () => {
    it('hay 5 tipos en el catálogo', async () => {
      if (!tenantId) return;
      const count = await prisma.cargoDirectoCatalogo.count({ where: { tenantId, activo: true } });
      expect(count).toBe(5);
    });
    ...
    it('viatico tiene zonas configuradas en JSON', async () => {
      ...
      const config = viatico.configJson as { zonas?: Array<{ codigo: string; monto: number }> };
    ...
    it('combustible_flete usa POR_UNIDAD_INPUT con $/km', async () => {
      ...
      const config = combustible.configJson as { precioPorUnidad?: number; unidad?: string };
  ...
  describe('Rutas de producción', () => {
    it('hay 5 rutas activas', async () => {
      if (!tenantId) return;
      const count = await prisma.ruta.count({ where: { tenantId, activo: true } });
      expect(count).toBe(5);
    });
  ...
  describe('Productos + configuración por paso', () => {
    ...
    it('Talonario tiene 2 rutas alternativas (emblocado preferido + abrochado)', async () => {
      ...
      const noPreferida = talonario.rutasAlternativas.find((r) => !r.esPreferida);
    ...
    it('Tarjetas Premium 300gr tiene config completa con maquina M-1 y materiales', async () => {
      ...
      const impresion = ruta.configPasos.find((c) => c.rutaPaso.familiaCodigo === 'impresion_por_hoja');
      ...
      expect(impresion!.slotsMateriales[0].slotCodigo).toBe('sustrato_principal');
    ...
    it('Vinilo blanco usa MOTOR_ELIGE_AUTO con criterio MAYOR_APROVECHAMIENTO', async () => {
      ...
      const impresion = ruta.configPasos.find((c) => c.rutaPaso.familiaCodigo === 'impresion_por_area');
      expect(impresion).toBeDefined();
      ...
      const candidatos = slot.materialesCandidatosJson as Array<{ variantId: string }>;
    ...
    it('Talonario emblocado tiene 3 pasos impresión con activación CONDICIONAL por capa', async () => {
      ...
      const pasosImpresion = ruta.configPasos.filter((c) => c.rutaPaso.familiaCodigo === 'impresion_por_hoja');
      ...
      const condicion2 = pasosImpresion[1].condicionActivacionJson as { '>='?: unknown[] };
    ...
    it('Talonario emblocado tiene paramsPaso "modoTalonarioIncompleto" en pre_prensa', async () => {
      ...
      const ruta = talonario.rutasAlternativas[0];
      const prePrensa = ruta.configPasos.find((c) => c.rutaPaso.familiaCodigo === 'pre_prensa');
      expect(prePrensa).toBeDefined();
      const params = prePrensa!.paramsPasoJson as { modoTalonarioIncompleto?: string };
  ...
  describe('Tab Precio (preservado)', () => {
    it('cada producto tiene precioConfigJson con método válido', async () => {
      ...
      const metodosValidos = new Set([
        'margen_variable', 'por_margen', 'precio_fijo',
        'fijado_por_cantidad', 'fijo_con_margen_variable',
        'variable_por_cantidad', 'precio_fijo_para_margen_minimo',
      ]);
