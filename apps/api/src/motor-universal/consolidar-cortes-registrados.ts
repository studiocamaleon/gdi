...
import { claveOperacionNesting, controlPrecedenciasNesting } from './precedencias-nesting-compuesto';
import type {
...
export function consolidarCortesRegistrados(
  ...
  precedencias: ControlConsolidacionProduccion = controlPrecedenciasNesting({ componentes }),
  ...
  for (const componente of componentes) {
    ...
    for (const paso of componente.pasos ?? []) {
      ...
      if (
        !paso.activado ||
        paso.familiaCodigo !== 'corte_laser' ||
        paso.tercerizado ||
        ...
        t.tarifaHora! < 0 ||
        (paso.materiales ?? []).some((m) => m.costoTotal !== 0)
      )
        continue;
  ...
  for (const [firma, miembros] of grupos) {
    ...
    if (
      ...
      continue;
    const operaciones = miembros.map((m) => claveOperacionNesting(m.componente.codigo, m.paso.rutaPasoId));
    const motivoPrecedencias = precedencias.motivoIncompatible(operaciones);
    if (motivoPrecedencias) {
      resultados.push({
        id, firmaVersion: 1, firmaCompatibilidad: firma,
        participantes: miembros.map(({ componente, paso }) => ({
          componenteCodigo: componente.codigo, productoId: componente.productoId,
          pasoClave: paso.configPasoId, rutaPasoId: paso.rutaPasoId,
          pasoNombre: paso.nombreVisible ?? 'Corte láser',
          piezas: [...new Set(paso.nestingResult!.placements.map((p) => p.pieceId))],
        })),
        independiente: { sustratos: origen.nestingResult.cantidadCalculada, aprovechamientoPct: origen.nestingResult.aprovechamientoPct },
        consolidado: { algoritmo: 'irregular-2d-bottom-left-v1', sustratos: origen.nestingResult.cantidadCalculada, aprovechamientoPct: origen.nestingResult.aprovechamientoPct, substrates: origen.nestingResult.substrates, placements },
        diferencia: { sustratos: 0, ahorroPct: 0, ahorroPotencial: false },
        aplicacion: { aplicado: false, motivoNoAplicado: motivoPrecedencias,
          costoMaterialIndependiente: 0, costoMaterialConsolidado: 0,
          costoPreparacionIndependiente: costoPreparacionAnterior,
          costoPreparacionConsolidado: costoPreparacionAnterior, ahorroCostoTotal: 0 },
      });
      continue;
    }
    ...
    const participantes = miembros.map(({ componente, paso }, index) => {
      ...
      const t = paso.tiempo!;
      const diferencia = asignado - costoPreparacion;
