...
function consultaPendiente() {
  ...
  const findMany = jest
    ...
    .mockReturnValue(resultado);
  const prisma = {
    ordenTrabajoItemPaso: { findMany },
  } as unknown as PrismaService;
  return {
    service: new SimulacionNestingColaService(prisma),
    findMany,
    resolver,
    rechazar,
  };
...
it('un fallo llega a ambas aperturas y permite reintentar con una nueva consulta', async () => {
  const { service, findMany, rechazar } = consultaPendiente();
...
it('separa tenants y máquinas, conserva el límite global y permite duplicados sin otra plaza', async () => {
  const { service, findMany, resolver } = consultaPendiente();
  const respuestas = Promise.allSettled([
