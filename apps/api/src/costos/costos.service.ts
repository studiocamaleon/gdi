...
export class CostosService {
  ...
  async getCentroConfiguracion(auth: CurrentAuth, id: string, periodo: string) {
    ...
    return {
      ...
      recursosMaquinaria: centro.recursos
        ...
        .map((recurso) =>
          this.toRecursoMaquinariaResponse(recurso.maquinariaPeriodo[0], recurso),
        )
  ...
  async upsertCentroRecursosMaquinaria(
    ...
    await this.prisma.$transaction(async (tx) => {
      const payloadResourceIds = payload.recursos.map(
        (item) => item.centroCostoRecursoId,
      );
      ...
      for (const item of payload.recursos) {
        ...
        await tx.centroCostoRecursoMaquinaPeriodo.upsert({
          ...
          create: {
            ...
            periodo: normalizedPeriodo,
            metodoDepreciacion: this.toPrismaMetodoDepreciacionMaquina(
  ...
  private async buildTarifaSnapshot(
    ...
    const costoMensualMaquinaria = centro.recursos
      .filter(
        (recurso) =>
          recurso.activo && recurso.tipoRecurso === TipoRecursoCentroCosto.MAQUINARIA,
      )
      .reduce(
        (acc, recurso) =>
    ...
    const costoMensualActivosFijos = centro.recursos
      .filter(
        (recurso) =>
          recurso.activo && recurso.tipoRecurso === TipoRecursoCentroCosto.ACTIVO_FIJO,
      )
      .reduce(
    ...
    return {
      ...
      resumenJson: {
        ...
        costoMensualGastosGenerales: this.decimalToNumber(
        ...
        costoMensualAbsorbidoReparto: this.decimalToNumber(
          costoMensualAbsorbidoReparto,
        ),
  ...
  private async computeRepartoPeriodo(
    auth: CurrentAuth,
    periodo: string,
    ...
    const totalBase = Array.from(baseByTarget.values()).reduce(
    ...
    for (const fuente of fuentes) {
      const divisor = useEqualWeights
        ? new Prisma.Decimal(centrosObjetivo.length)
        : totalBase;
      if (!divisor.gt(0)) {
        continue;
      }
  ...
  private buildAdvertencias(
    ...
    if (
      recursosMaquinariaActivos.some(
        (recurso) => !recurso.maquinariaPeriodo[0],
      )
    ) {
      advertencias.push(
  ...
  private computeCapacidad(
    ...
    const capacidadHoraMaquina = recursos
      .filter((recurso) => recurso.tipoRecurso === TipoRecursoCentroCosto.MAQUINARIA)
      .reduce((acc, recurso) => {
    ...
    const capacidadHoraHombre = recursos
      .filter((recurso) => recurso.tipoRecurso === TipoRecursoCentroCosto.EMPLEADO)
      .reduce((acc, recurso) => {
        const porcentaje = recurso.porcentajeAsignacion ?? new Prisma.Decimal(0);
        return acc.plus(horasBaseMes.mul(porcentaje).div(new Prisma.Decimal(100)));
      }, new Prisma.Decimal(0));
    const capacidadAuto =
  ...
  private async validateRecursos(
    ...
    if (!centro) {
      throw new NotFoundException(`No existe el centro de costo ${centroCostoId}`);
    }
    ...
    for (const recurso of recursos) {
      ...
      if (
        ...
        if (
          recurso.valorMensual === undefined ||
  ...
  private toRecursoMaquinariaResponse(
    item:
      | Prisma.CentroCostoRecursoMaquinaPeriodoGetPayload<object>
      | undefined,
    recurso: {
      id: string;
      periodo: string;
    ...
    return {
      ...
      amortizacionMensual: item
        ...
        : Number(
            (Math.max(0, valorCompra - valorResidual) / Math.max(1, vidaUtilMeses)).toFixed(2),
          ),
  ...
  private computeMaquinariaCosteo(
    ...
    const baseDepreciable = rawBaseDepreciable.gt(0)
      ? rawBaseDepreciable
      : new Prisma.Decimal(0);
    const amortizacionMensual = baseDepreciable.div(new Prisma.Decimal(vidaUtilMeses));
  ...
  private fromPrismaTipoGastoGeneral(
    ...
    > = {
      ...
      [TipoGastoGeneralCentroCosto.ALQUILER]:
        TipoGastoGeneralCentroCostoDto.alquiler,
      [TipoGastoGeneralCentroCosto.OTRO]:
        TipoGastoGeneralCentroCostoDto.otro,
    };
  ...
  private fromPrismaMetodoDepreciacionMaquina(
    metodo: MetodoDepreciacionMaquina,
  ) {
