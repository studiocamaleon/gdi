...
function setup({
  confirmado = false,
  ganaConfirmacion = true,
  item = false,
} = {}) {
  const archivo = {
    id: 'archivo-1',
    tenantId: 'tenant-1',
    estado: confirmado ? 'LISTO' : 'PENDIENTE',
    nombreOriginal: 'arte-final.pdf',
    mimeType: 'application/pdf',
    bytes: 20n,
    key: 'tenant-1/arte.pdf',
    scope: item ? 'ORDEN_ITEM' : 'ORDEN',
    ordenId: item ? null : 'ot-1',
    ordenItemId: item ? 'item-1' : null,
    createdAt: new Date(),
  };
  const tx = {
    ...
    archivo: {
      findFirst: jest.fn().mockResolvedValue({
        ...archivo,
        estado: ganaConfirmacion ? 'PENDIENTE' : 'LISTO',
      }),
      aggregate: jest.fn().mockResolvedValue({
        _sum: { bytesReservados: 0n },
        _count: { _all: 0 },
      }),
      updateMany: jest
        .fn()
        .mockResolvedValue({ count: ganaConfirmacion ? 1 : 0 }),
      findFirstOrThrow: jest
        .fn()
        .mockResolvedValue({ ...archivo, estado: 'LISTO' }),
    },
    tenant: {
      update: jest.fn(),
      findUnique: jest
        .fn()
        .mockResolvedValue({ bytesArchivos: 0n, cuotaBytesArchivos: null }),
    },
    ordenTrabajoItem: {
      findFirst: jest.fn().mockResolvedValue({ ordenId: 'ot-1' }),
    },
  };
  const prisma = {
    archivo: { findFirst: jest.fn().mockResolvedValue(archivo) },
    $transaction: jest.fn((cb: (db: typeof tx) => unknown) => cb(tx)),
  };
  const storage = {
    cabecera: jest
      .fn()
      .mockResolvedValue({ bytes: 20, contentType: 'application/pdf' }),
    leerCabecera: jest.fn().mockResolvedValue(Buffer.from('%PDF-')),
  };
...
describe('Confirmación de archivos y actividad', () => {
  it.each([false, true])(
    'registra la subida junto con el estado y los bytes; archivo de ítem: %s',
    async (item) => {
      const { service, auth, tx, eventos } = setup({ item });
      await service.confirmar(auth, 'archivo-1');
      expect(tx.archivo.updateMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { id: 'archivo-1', tenantId: 'tenant-1', estado: 'PENDIENTE' },
        }),
      );
      expect(tx.tenant.update).toHaveBeenCalledTimes(1);
      expect(eventos.publicarDesdeAuth).toHaveBeenCalledWith(
        auth,
        expect.objectContaining({
          tipo: 'archivo.orden_confirmado',
          href: '/produccion/ordenes/ot-1',
          mensaje: 'arte-final.pdf',
        }),
        tx,
      );
    },
